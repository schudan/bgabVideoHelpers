...
VOB_DIR="/media/raid/videos/rips/vobs" # Location to store vobs
ATV2_DIR="/media/raid/videos/rips/completed" # Location to place ATV2 encoded videos
IPAD_DIR="/media/raid/videos/rips/ipad" # Location to place iPad encoded videos
DVD_DIR="/media/dvd" # Mount location of dvd
DVD_DEV="/dev/sr0" # DVD Device
LOCK_FILE="/media/raid/videos/rips/raw dvd/ripdvd.lock" # Lock File
...
HandBrakeCLI -i "${VOB_DIR}/${DVD_NAME}/" -o "${ATV2_DIR}/${ATV2_NAME}.mp4" --preset="Normal"
#HandBrakeCLI -i "${VOB_DIR}/${DVD_NAME}/" -o "${ATV2_DIR}/${ATV2_NAME}.mp4" -a 1,2,3 --srt-lang eng,deu --srt-file --preset="Normal"
if [ $? -ne 0 ]; then
   # encoding failed
