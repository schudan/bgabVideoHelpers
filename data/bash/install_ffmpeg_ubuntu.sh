...
sudo apt-get update
sudo apt-get -y install build-essential checkinstall git  libfdk_aac-dev libgpac-dev \
...
cd ffmpeg
./configure --enable-gpl --enable- libfdk_aac --enable-libmp3lame --enable-libopencore-amrnb \
